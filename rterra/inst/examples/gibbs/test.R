...
invisible(gc())
resLuaJIT <- replicate(R,{invisible(gc);system.time(terra("doGibbsJIT",a))})["elapsed",]

invisible(gc())
resTerra <- replicate(R,{invisible(gc);system.time(terra("doGibbsTerra",a))})["elapsed",]
cat(".Call to Terra code\n")
print(res)

invisible(gc())
resTerraP <- replicate(R,{invisible(gc);system.time(terra("doGibbsTerraParallel",a,2L))})["elapsed",]
...
library(lattice)
j <- do.call(rbind,lapply(list("resC","resLuaJIT"),function(a){
  b <- get(a)
  data.frame(type=a, time=b)
}))
bwplot( type ~ time, data=j, xlab='time(sec)', ylab='method',scale=list(x=list(tick.number=20)))
